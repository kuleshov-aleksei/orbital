...
func (r *DebugLogRepository) Create(log *models.DebugLog) error {
	result, err := r.db.Exec(
		`INSERT INTO debug_logs (user_id, username, created_at, log_filename) VALUES (?, ?, ?, ?)`,
		log.UserID, log.Username, log.CreatedAt, log.LogFilename,
	)
	if err != nil {
		return err
	}
...
func (r *DebugLogRepository) GetByID(id int64) (*models.DebugLog, error) {
	...
	err := r.db.QueryRow(
		`SELECT id, user_id, username, created_at, log_filename FROM debug_logs WHERE id = ?`,
		id,
	).Scan(&log.ID, &log.UserID, &log.Username, &createdAt, &log.LogFilename)
...
func (r *DebugLogRepository) GetAll() ([]*models.DebugLog, error) {
	rows, err := r.db.Query(
		`SELECT id, user_id, username, created_at, log_filename FROM debug_logs ORDER BY created_at DESC`,
	)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := rows.Scan(&log.ID, &log.UserID, &log.Username, &createdAt, &log.LogFilename)
		if err != nil {
			return nil, err
		}
