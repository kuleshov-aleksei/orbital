...
func (r *RoomRepository) Create(room *models.Room) error {
	_, err := r.db.Exec(
		`INSERT INTO rooms (id, name, owner_id, max_users, type, created_at, category_id, sort_order, world) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		room.ID, room.Name, room.OwnerID, room.MaxUsers, room.Type, room.CreatedAt, room.Category, room.SortOrder, room.World,
	)
	return err
}
...
func (r *RoomRepository) GetByID(id string) (*models.Room, error) {
	...
	err := r.db.QueryRow(
		`SELECT id, name, owner_id, max_users, type, created_at, category_id, sort_order, world FROM rooms WHERE id = ?`,
		id,
	).Scan(&room.ID, &room.Name, &ownerID, &room.MaxUsers, &room.Type, &room.CreatedAt, &categoryID, &room.SortOrder, &room.World)
...
func (r *RoomRepository) Update(room *models.Room) error {
	_, err := r.db.Exec(
		`UPDATE rooms SET name = ?, owner_id = ?, max_users = ?, type = ?, category_id = ?, sort_order = ?, world = ? WHERE id = ?`,
		room.Name, room.OwnerID, room.MaxUsers, room.Type, room.Category, room.SortOrder, room.World, room.ID,
	)
	return err
}
...
func (r *RoomRepository) GetAll() ([]*models.Room, error) {
	rows, err := r.db.Query(`SELECT id, name, owner_id, max_users, type, created_at, category_id, sort_order, world FROM rooms ORDER BY sort_order ASC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&room.ID, &room.Name, &ownerID, &room.MaxUsers, &room.Type, &room.CreatedAt, &categoryID, &room.SortOrder, &room.World)
		if err != nil {
			return nil, err
		}
...
func (r *RoomRepository) GetByCategory(categoryID string) ([]*models.Room, error) {
	rows, err := r.db.Query(
		`SELECT id, name, owner_id, max_users, type, created_at, category_id, sort_order, world FROM rooms WHERE category_id = ? ORDER BY sort_order ASC`,
		categoryID,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		...
		err := rows.Scan(&room.ID, &room.Name, &ownerID, &room.MaxUsers, &room.Type, &room.CreatedAt, &catID, &room.SortOrder, &room.World)
		if err != nil {
			return nil, err
		}
