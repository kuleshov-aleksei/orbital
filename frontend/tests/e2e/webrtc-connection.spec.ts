...
test('two users establish WebRTC audio connection', async ({ browser, request }) => {
  ...
  const hasConnectionStates = metricsA?.includes('disconnected') || metricsA?.includes('connected') ||
                            metricsB?.includes('disconnected') || metricsB?.includes('connected')
  ...
  await pageA.screenshot({ path: 'test-results/alice-debug.png', fullPage: true })
  await pageB.screenshot({ path: 'test-results/bob-debug.png', fullPage: true })
