...
test("setup: create categories, rooms and 7 regular users", async ({ request }) => {
  ...
  if (!loginRes.ok()) {
    console.log("Admin user not found. Create AdminUser manually and grant admin rights, then re-run.")
    return
  }

  const loginData = (await loginRes.json()) as { token: string; user: { id: string; nickname: string; role: string } }
  ...
  const existingRooms = (await roomsResponse.json()) as Array<{ name: string; category_name: string }>
